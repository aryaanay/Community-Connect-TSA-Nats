      ...
      <div className="group/ticker flex items-center gap-5 overflow-hidden">
        <div className="audience-marquee-track flex min-w-max items-center gap-4 whitespace-nowrap group-hover/ticker:[animation-play-state:paused]">
          {repeated.map(({ Icon, label, note }, i) => (
            <div
              key={`${label}-${i}`}
              className="group/item inline-flex items-center gap-3 rounded-2xl border border-sky-200/80 bg-white/75 px-5 py-3 text-left shadow-sm backdrop-blur-xl transition-all duration-300 hover:border-sky-300 hover:bg-white hover:shadow-md hover:shadow-sky-500/10"
            >
