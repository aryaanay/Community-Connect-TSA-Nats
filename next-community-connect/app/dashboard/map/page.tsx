...
function makeSvgIcon(color: string, active = false) {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size + 8}" viewBox="0 0 30 38"><filter id="s"><feDropShadow dx="0" dy="2" stdDeviation="2" flood-opacity="0.35"/></filter><circle cx="15" cy="15" r="13" fill="${color}" filter="url(#s)" opacity="${active ? 1 : 0.92}"/><circle cx="15" cy="15" r="6" fill="white" opacity="0.9"/><polygon points="15,38 9,24 21,24" fill="${color}" opacity="${active ? 1 : 0.92}"/></svg>`
...
export default function DashboardMapPage() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f0f7ff]">
      ...
      {/* Map + panel — isolate keeps Leaflet z-indices from escaping above AI widget */}
